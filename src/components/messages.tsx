...
export const Message = ({
  ...
  const { parentMessageId, onClose, onOpenProfile, openMessage } = usePanel();
      ...
      <>
        ...
        <div
          className={cn(
            ...
            isDeletingMessage && 'bg-rose-500/50 transform transition-all scale-y-0 origin-bottom duration-300'
          )}>
          <div className='flex items-start gap-2'>
            <Hint label={formatFullTime(new Date(createdAt))}>
              <button className='text-xs text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity w-[40px] leading-[22px] text-center hover:underline'>
          ...
          {!isEditing && (
            <Toolbar
              isAuthor={isAuthor}
              isPending={isPending || isUpdatingMessage || isDeletingMessage}
          ...
          {isEditing ? (
            ...
            <div className='flex flex-col w-full overflow-hidden'>
              <div className='text-sm'>
                ...
                <span>&nbsp;&bull;&nbsp;</span>
                <Hint label={formatFullTime(new Date(createdAt))}>
                  <button className='text-xs text-muted-foreground hover:underline'>{format(new Date(createdAt), 'hh:mm a')}</button>
                </Hint>
              </div>
              <Renderer value={body} />
              <Thumbnail url={image} />
