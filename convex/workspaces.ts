...
const generateCode = () => {
  const joinCode = Array.from({ length: 6 }, () => '0123456789abcdefghijklmnopqrstuvwxyz'[Math.floor(Math.random() * 36)]).join('');
  return joinCode;
};
...
export const join = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingMember) throw new Error('already an active member of this workspace');
