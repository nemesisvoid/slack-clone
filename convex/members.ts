...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (member.role === 'admin') throw new Error('admins cannot be removed');

    if (args.id === currentMember._id && currentMember.role === 'admin') throw new Error('cannot remove self if self is an admin');
